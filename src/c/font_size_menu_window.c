...
static char s_radio_button_menu_items[FONT_SIZE_RADIO_BUTTON_WINDOW_NUM_ROWS][7] = { "Normal", "Large" };

static uint16_t get_num_rows_callback(MenuLayer *menu_layer, uint16_t section_index, void *context) {
  return FONT_SIZE_RADIO_BUTTON_WINDOW_NUM_ROWS;
}
...
static void draw_row_callback(GContext *ctx, const Layer *cell_layer, MenuIndex *cell_index, void *context) {
    ...
    GPoint p = GPoint(bounds.size.w - (PBL_IF_ROUND_ELSE(6, 3) * FONT_SIZE_RADIO_BUTTON_WINDOW_RADIO_RADIUS), (bounds.size.h / 2));
    ...
    graphics_draw_circle(ctx, p, FONT_SIZE_RADIO_BUTTON_WINDOW_RADIO_RADIUS);
    if(cell_index->row == s_radio_button_current_selection) {
      // This is the selection
      graphics_fill_circle(ctx, p, FONT_SIZE_RADIO_BUTTON_WINDOW_RADIO_RADIUS - 3);
    }
}
...
static void select_callback(struct MenuLayer *menu_layer, MenuIndex *cell_index, void *callback_context) {
  ...
  persist_write_bool(CONFIG_KEY_LARGE_FONT, (cell_index->row == 1));
